...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(stopmusic:)
                                                 name:UIApplicationWillResignActiveNotification //액티브상태아님
                                               object:nil];
    //앱이 액티브상태로 진입
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(playmusic:)
                                                 name:UIApplicationDidBecomeActiveNotification //액티브상태로돌아옴
                                               object:nil];
}

//음악 정지
- (void)stopmusic:(NSNotification *)noti
{
    if ([[self player] isPlaying]) {
        [self setShouldcontinueplay:YES]; //플레이를 계속한다 - yes
        [self.player pause]; //정지
    }
}
...
- (void)playmusic:(NSNotification *)noti
{
    if ([self shouldcontinueplay]) {
        [self setShouldcontinueplay:NO]; //플레이를 계속한다 - no
        [self.player play]; //재생
    }
}
