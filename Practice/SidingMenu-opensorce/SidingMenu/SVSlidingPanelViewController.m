...
//containerview = viewcontroller에는 controller를 올릴수 없다.
//하지만 containerview위에는 viewcontroller를 올릴 수 있다.
...
- (void)viewDidLoad {
    ...
    //autoresizingMask = 상위뷰가 크기나 위치가 변경될경우 상위뷰에 따라 자동을 리사이징된다.
    self.view.backgroundColor = [UIColor whiteColor];
    self.view.autoresizingMask = UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
    
    //leftPanelFrame < 뷰의 전체크기를 넣는다.
    //leftPanelFrame.size.width < 아래 메서드 실행(메인뷰 전체프레임넓이 * 0.5)
    CGRect leftPanelFrame = self.view.bounds;
    leftPanelFrame.size.width = [self leftSlidingPanelVisibleWidth];
    
    self.leftPanelContainerView.frame = leftPanelFrame; // < uiview
...
- (void)styleForContainerView:(UIView *)containerView animationOption:(BOOL)animated
{
    ...
    //
    containerView.layer.shadowPath = (__bridge CGPathRef _Nullable)((__bridge id _Nullable)([shadowPath CGPath]));
